...
(async()=>{const browser=await chromium.launch({executablePath:process.env.CHROME_PATH||'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true});try{
 ...
 const result=await page.evaluate(()=>{
  ...
  const md=dashaRoot.find(p=>p.lord==='Mercury'&&p.a>chart.birthMs);if(Math.abs(scorePeriod(md,0)-0.3045283084587375)>1e-12)throw Error('Raw Mercury score changed');
  const ad=childPeriods(md).find(p=>p.lord==='Rahu');if(Math.abs(lifetimePercentile(scorePeriod(ad,0),0)-11.861791353202237)>1e-9)throw Error('Lifetime reference changed');
