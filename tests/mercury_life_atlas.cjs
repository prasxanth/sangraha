...
const crypto=require('node:crypto'),golden=require('./fixtures/mercury_v5_reference.json');
...
(async () => {
 ...
 try {
  ...
  assert.equal(hash(actual.results),golden.resultsSha256,'All 7,290 results match the independently captured v5 source');
  ...
  assert(await page.evaluate(()=>DATA.every(pd=>DIM_ORDER.every(k=>sdByParent.get(pd.ad+'/'+pd.pd).every(sd=>(pdResult(pd,k).heat===dimResult(sd,k).parent&&pdResult(pd,k).score===dimResult(sd,k).parentScore))))), 'Every parent view matches v5 parent heat');
  assert(await page.evaluate(()=>DATA.every(r=>pdResult(r,'career').heat===r.heat&&pdResult(r,'career').rationale===r.assessment)), 'All 81 original Career anchors and assessments must be restored');
  ...
  assert(await page.evaluate(()=>modal.scrollWidth<=modal.clientWidth+1),'Expanded v5 transit evidence fits a narrow phone');
  await page.screenshot({path:'/tmp/mercury-v5-evidence.png'});
