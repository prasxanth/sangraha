...
const crypto=require('node:crypto'),golden=require('./fixtures/mercury_v4_reference.json');
...
(async () => {
 ...
 try {
  ...
  assert.equal(hash(actual.results),golden.resultsSha256,'All 7,290 results match the independently captured v4 source');
  ...
  assert(await page.evaluate(()=>DATA.every(pd=>DIM_ORDER.every(k=>sdByParent.get(pd.ad+'/'+pd.pd).every(sd=>(pdResult(pd,k).heat===dimResult(sd,k).parent&&pdResult(pd,k).score===dimResult(sd,k).parentScore))))), 'Every parent view matches v4 parent heat');
  assert(await page.evaluate(()=>DATA.some(r=>pdResult(r,'career').heat!==r.heat)), 'Career must be rescored');
  assert(await page.evaluate(()=>SD_DATA.every(r=>DIM_ORDER.every(k=>{const d=dimResult(r,k),base=pdResult(r,k).raw;return Math.abs(d.heat-d.parent)<=(Math.abs(d.raw-base)>=.62?2:1)}))), 'V4 adaptive one/two-band limit');
  ...
  assert.deepEqual(hierarchyExample,{parent:2,parentScore:23,heat:2,score:32,intensity:'LOW'});
  await page.evaluate(()=>{setFocusDimension('career');showPeriod(DATA.findIndex(r=>r.ad==='Venus'&&r.pd==='Mercury'))});
  assert.equal(await page.locator('#pdPanel [data-parent-score]').textContent(),'+23');
  await page.locator('#sdTab').click();await page.selectOption('#sdSelect','0');
  assert.equal(await page.locator('#sdPanel [data-parent-score]').textContent(),'+23');
  assert((await page.locator('#sdSelection .support-intensity').textContent()).includes('+32'));
  ...
  assert(await page.evaluate(()=>modal.scrollWidth<=modal.clientWidth+1),'Expanded v4 transit evidence fits a narrow phone');
  await page.screenshot({path:'/tmp/mercury-v4-evidence.png'});
