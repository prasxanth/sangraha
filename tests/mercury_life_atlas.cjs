...
const crypto=require('node:crypto'),golden=require('./fixtures/mercury_v2_reference.json');
...
(async () => {
 ...
 try {
  ...
  assert.equal(hash(actual.results),golden.resultsSha256,'All 7,290 results match the independently captured v2 source');
  ...
  assert(await page.evaluate(()=>DATA.every(pd=>DIM_ORDER.every(k=>sdByParent.get(pd.ad+'/'+pd.pd).every(sd=>pdResult(pd,k).heat===dimResult(sd,k).parent)))), 'Every parent view matches v2 parent heat');
