...
(async()=>{const browser=await chromium.launch({executablePath:process.env.CHROME_PATH||'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true});try{
 ...
 const bands=await page.evaluate(()=>{
  ...
  const directions=[[.75,1.15,1.15],[.35,1.15,.5],[.35,.5,.5],[.1,1.15,1.15],[.1,1.15,.5],[.1,.5,.5],[-.35,1.15,.5],[-.35,.5,.5]].map(x=>directionFromScores(...x).action);
 ...
 const dc=page.locator('#directionHeat .direction-cell').first();await dc.scrollIntoViewIfNeeded();await page.waitForTimeout(250);await dc.tap();await page.locator('.heat-preview-evidence > summary').click();assert((await page.locator('.heat-magnifier').innerText()).includes('Direction'));assert.equal(await page.locator('.heat-magnifier .metric-number').count(),4);await page.keyboard.press('Escape');
 await page.locator('#directionHeat button[data-view="details"]').click();assert((await page.locator('#directionHeat').innerText()).includes('Lifetime-relative support percentile'));assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
