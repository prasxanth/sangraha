...
(async()=>{const browser=await chromium.launch({executablePath:process.env.CHROME_PATH||'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true});try{
 ...
 await page.locator('#cycleMetric-scale').click();await page.locator('#scaleHeat .touch-heat-cell').first().waitFor();
 ...
 await page.locator('#cycleMetric-direction').click();if(!await page.locator('#directionComparison > details').evaluate(e=>e.open))await page.locator('#directionComparison > details > summary').click();await page.locator('#directionHeat .direction-cell').first().waitFor();
 ...
 await page.locator('#cycleMetric-scale').click();await page.evaluate(()=>{selectedSystemIds.delete('Vedic');renderScaleExplorer()});await page.waitForFunction(()=>document.querySelector('#scaleHeat').textContent.includes('Select Vedic'));
