...
(async()=>{const browser=await chromium.launch({executablePath:process.env.CHROME_PATH||'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true});try{
 ...
 await page.locator('#directionComparison > details > summary').click();await page.locator('#directionHeat .direction-cell').first().waitFor();
 ...
 await page.locator('#directionHeat button[data-view="details"]').click();assert((await page.locator('#directionHeat').innerText()).includes('Categorical direction'));assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 ...
 assert.deepEqual(errors,[]);console.log('PASS: exact bucket boundaries and eight direction categories, existing expression values, all-domain duration consistency, unchanged support/activation, absolute defaults, mobile map/preview/details/key and absent-Vedic state.');
