...
(async()=>{const browser=await chromium.launch({executablePath:process.env.CHROME_PATH||'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',headless:true});try{
 ...
 assert.deepEqual(audit.bands,['Trough','Trough','Below average','Below average','Typical','Typical','Above average','Above average','Peak','Peak']);
 ...
 assert((await page.locator('.heat-magnifier').innerText()).includes('percentile'));assert((await page.locator('.heat-magnifier').innerText()).includes('Absolute support'));await page.locator('.heat-preview-evidence > summary').click();const details=await page.locator('.heat-magnifier').innerText();assert(details.includes('Absolute support')&&details.includes('Activation')&&details.includes('Scale / reach'));await page.locator('.heat-magnifier').screenshot({path:'/tmp/kala-lifetime-preview.png'});await page.keyboard.press('Escape');
 ...
 await page.evaluate(()=>{selectedSystemIds.delete('Vedic');renderDirectionExplorer()});await page.locator('#directionHeat .explorer-item').first().waitFor();assert((await page.locator('#directionHeat').innerText()).includes('percentile'));
