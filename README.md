...
The supplied whole-life model covers Career, Wealth, Marriage, Family, Children, Mind, Vitality, Learning, Spiritual and Reinvention. All 729 period boundaries are preserved; transit snapshots, contacts and summaries are replaced with the supplied v3 true-node Lahiri data. The v4 model with PD as the dominant layer recalculates all ten dimensions, including Career, using centered chart-specific profiles, dasha-pair interactions, midpoint transits, Moon-relative houses for Mind, stronger conjunction/opposition contacts, and a light fast-SD-lord transit modifier. Parent weights are 10% MD, 24% AD, 66% PD and 45% of the AD–PD interaction. SD uses 18% of its centered profile, 30% PD–SD interaction and 8% AD–SD interaction plus transit evidence. SD support normally stays within one heat band of its parent; a two-band allowance requires an absolute short-term adjustment of at least 0.62; extreme ratings additionally require convergent evidence or an exceptionally strong dasha signal. Intensity is calculated separately and appears as 1–4 bars in the current overview and SD comparison map, with a named category and support index in details. The old stored career ratings no longer determine displayed support. Parent details show only the dasha baseline; audited transit evidence is available at the SD midpoint. Parent scores normalize the baseline by 1.10; SD scores normalize the composite by 1.35. The Guide attributes the retained v3 audit foundation and v4 sensitivity findings to the supplied source. PD baselines are not averages of their children. The app evaluates the embedded model offline; it does not recalculate ephemerides or natal charts.
...
**Validation:** `tests/mercury_dasha_atlas.cjs`, `tests/mercury_sookshma.cjs`, `tests/mercury_life_atlas.cjs`, and `tests/mercury_current_period.cjs` use Playwright with local Chrome. The life-atlas test optionally accepts the supplied `mercury_md_whole_life_heatmap_v4_pd_dominant.html` path to compare all records and all 7,290 dimension results against the source. A checked-in v4 reference fixture also verifies all 7,290 outputs without requiring the external source file.
